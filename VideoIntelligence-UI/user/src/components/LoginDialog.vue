...
const agreementContents: Record<'service' | 'privacy', string[]> = {
  service: [
    '欢迎使用智能化视频分析平台（以下简称“本平台”）。本协议为您与本平台之间就使用本平台服务所订立的协议。',
    '1. 账号说明：您可通过微信、QQ、GitHub 等第三方账号扫码登录本平台，登录成功后即视为您已阅读并同意本协议的全部内容。',
    '2. 使用规范：您应当妥善保管账号信息，不得利用本平台从事任何违反法律法规、侵犯他人合法权益的活动。',
    '3. 服务变更：本平台有权根据业务需要调整、暂停或终止部分或全部服务，并将以适当方式提前告知。',
    '【说明】以上内容为演示用的假数据，正式上线前将替换为法务确认后的协议正文。',
  ],
  privacy: [
...
const handleGithubLogin = async () => {
  ...
  try {
    ...
    githubLoading.value = false
    const message = error as { msg?: string; message?: string }
    ElMessage.error(message?.msg || message?.message || '获取 GitHub 授权地址失败，请稍后重试')
  }
}
...
<template>
  ...
  >
    ...
    <div v-if="isGithubChannel" class="github-panel">
      ...
      <p class="github-desc">点击下方按钮将跳转到 GitHub 官方页面完成授权，授权成功后自动返回本平台</p>
      <el-button
        class="github-button"
        type="primary"
        size="large"
    ...
    <p class="dialog-tip">GitHub 为跳转式授权登录；微信 / QQ 仍为演示二维码（假数据，未接入后端接口）</p>
  </el-dialog>
